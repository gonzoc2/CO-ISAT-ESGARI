...
def tabla_expandible_comp(df,df_ly,df_ppt, cat, mes, pro, dic, dic_ly, dic_ppt, key_prefix):
        ...
        df_combinado["Alcance_LY"] = df_combinado.apply(
            lambda row: (row["YTD"] / row["LY"] * 100) if row["YTD"] > 0 and row["LY"] != 0 else 0, axis=1
        )
        df_combinado["Alcance_PPT"] = df_combinado.apply(
            lambda row: (row["YTD"] / row["PPT"] * 100) if row["YTD"] > 0 and row["PPT"] != 0 else 0, axis=1
        )
...
if selected == "Resumen":
   ...
   # Función para formatear celdas en pesos
    ...
    def generar_tabla_con_estilo(df):
        # Función para aplicar colores condicionales solo a los valores
        def aplicar_estilos(val):
            if isinstance(val, (int, float)):
                if val > 0:
                    return 'background-color: #d4edda; color: black;'  # Verde claro con texto negro
                elif val < 0:
                    return 'background-color: #f8d7da; color: black;'  # Rojo claro con texto negro
            return 'color: black;'  # Texto negro para valores normales

        # Crear estilos para encabezados y filas alternadas
        estilos_filas = [
            {'selector': 'thead th', 'props': 'background-color: #001f3f; color: white; font-weight: bold; font-size: 14px;'},
            {'selector': 'tr', 'props': 'background-color: white; color: black;'},  # Fondo blanco y texto negro
            {'selector': 'tr:nth-child(even)', 'props': 'background-color: #f9f9f9; color: black;'}  # Gris claro y texto negro
        ]

        # Convertir el DataFrame en HTML con estilos personalizados
        html = df.style.applymap(aplicar_estilos).set_table_styles(estilos_filas).format(
            na_rep='-', precision=2
        ).set_properties(
            **{'font-size': '12px', 'color': 'black'}  # Asegurar texto negro en toda la tabla
        ).render()
    ...
    tabs = ['Comparativa Horizontal', 'Comparativa Vertical']
    st.tabs(tabs)
    ...
    def pe(df, codigo_pro, ultimos_meses, pro, ultimo_mes):
        ...
        df_pe_mese = df_pe_mes[df_pe_mes['Mes_A'].isin(ultimos_meses)]
        ...
        df_pe_meses_fijos_mes = df_pe_mese.groupby(['Categoria_A', 'Clasificacion_A', 'Cuenta_Nombre_A', 'Mes_A'])['Neto_A'].sum().reset_index()
        df_pe_meses_variables_mes = df_pe_mese.groupby(['Categoria_A', 'Clasificacion_A', 'Cuenta_Nombre_A'])['Neto_A'].sum().reset_index()
        ...
        oh_total_mes_actual = ingreso_total * oh_obj
        ...
        <table>
            ...
            <tr>
                <td>Costos variables del mes actual con OH objetivo</td>
                <td>${ingreso_total:,.2f}</td>
